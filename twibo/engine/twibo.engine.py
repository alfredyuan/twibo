...
class TwitterParser:
	...
	def ShortLink(source):
		...
		try:
			toRet = json.loads(r.text)['long-url']
		except KeyError:
			toRet = ''
	...
	def process(source):
		...
		for url in urls:
			toReplace = TwitterParser.ShortLink(url)
			if toReplace.strip() != '':
				source = re.sub(url, toReplace , source, count=1)
...
class TwiboEngine:	
	...
	def clean(self):
		with transaction.manager:
			DBSession.query(TwiboEngineLog).delete()
		self.__log('CLEANED')
	...
	def process(self):
		...
		for c in c_:
			...
			#customer = DBSession.query(Customer).filter_by(id=c.id).one()
			...
			for t in c.tweets:
				if not t.weibo:
					...
					if w:
						with transaction.manager:
							DBSession.add(Weibo(w, t.id, c.id))
						#DBSession.flush()
...
if __name__ == '__main__':
	...
	if len(sys.argv)==1 or (len(sys.argv)>1 and sys.argv[1].lower() == 'run'):
		engine.run()
